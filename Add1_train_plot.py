...
for w in dictionary:
    if w not in ['<S>', '<E>']:
        count_word_context = findcount(context, w, path_name + modified_corpus_name)
        count_context_allword = 0
        co = 0
        for word in dictionary:
            count_context_allword = count_context_allword + findcount(context, word, path_name + modified_corpus_name)
        probdictionaryunsmo[w] = calProb(count_word_context,count_context_allword,vocab_size,1)
...
for w in dictionary:
    if w not in ['<S>', '<E>']:
        ...
        for word in dictionary:
            count_context_allword = count_context_allword + findcount(context, word, path_name + modified_corpus_name)
        probdictionarysmo[w] = calProb(count_word_context,count_context_allword,vocab_size,2)
...
plt.rcdefaults()
fig, ax = plt.subplots()
ax.barh(x, probsm,width, color='g' ,align='center',label='Add-1')
ax.barh(x+width, probusm, width, color='b', align='center',label='Unsmoothed')
...
ax.set_title('Bar Graph for Unsmoothed and Add-1 Bigram with context=\'READ\'')
